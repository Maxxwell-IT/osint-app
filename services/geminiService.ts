...
export async function investigateTarget(target: string): Promise<{ data: OSINTResult; sources: GroundingChunk[] } | null> {
    const prompt = `
        ...
        Ваше завдання — провести всебічне розслідування щодо вказаної цілі: "${target}". Якщо ціль виглядає як конкретний елемент даних з попереднього розслідування (наприклад, email або ім'я користувача), зосередьтеся на пошуку нових зв'язків та інформації, що випливають *саме з цього елемента*, щоб розкрити приховані закономірності.
