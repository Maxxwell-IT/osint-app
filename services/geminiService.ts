...
export async function investigateTarget(target: string): Promise<{ data: OSINTResult; sources: GroundingChunk[] } | null> {
    const prompt = `
        ...
        - Data Breaches: Дуже ретельно перевірте, чи з'являлася ціль у відомих витоках даних. Для кожного витоку вкажіть назву, дату та, що найважливіше, надайте вичерпний список ВСІХ типів скомпрометованих даних у полі 'compromised_data'. Наприклад: ["email", "password (hashed)", "IP address", "full name", "physical address"]. Не обмежуйтесь загальними фразами.
        - Forum Mentions: Знайдіть згадки на форумах. Для кожної згадки надайте назву форуму та URL. У полі 'post_snippet' надайте максимально інформативний та контекстуально багатий уривок з повідомлення, де згадується ціль. Уривок повинен давати чітке уявлення про те, в якому контексті було зроблено згадку. Це може бути пряма цитата або детальний переказ.
